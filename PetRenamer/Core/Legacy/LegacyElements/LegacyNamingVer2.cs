...
{
    internal override void OnPlayerAvailable(PlayerCharacter playerData, int detectedVersion)
    {
        if (detectedVersion != 2) return;
        if (PluginLink.Configuration.users!.Length == 0) return;

        SerializableUser newSerializableUser = new SerializableUser((SerializableNickname[])PluginLink.Configuration.users.Clone(), playerData.Name.ToString(), (ushort)playerData.HomeWorld.Id);
