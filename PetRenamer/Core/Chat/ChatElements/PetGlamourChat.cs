...
internal class PetGlamourChat : ChatElement
{
    ...
    void SetClassJobToSkeleton(int classJob, int skeleton)
    {
        ...
        if (classJob == -2) localUser.SerializableUser.mainSmnrSkeleton = skeleton == -1 ? PluginConstants.baseSummonerSkeleton : skeleton;
        if (classJob == -3) localUser.SerializableUser.mainSchlrSkeleton = skeleton == -1 ? PluginConstants.baseScholarSkeleton : skeleton;
    }
