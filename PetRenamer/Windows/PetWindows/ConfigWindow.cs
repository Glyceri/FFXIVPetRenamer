...
public class ConfigWindow : PetWindow
{
    ...
    public override void OnDraw()
    {
        ...
        {
            if (BeginElementBox("Third Party WARNING"))
            {
    ...
    bool BeginElementBox(string title)
    {
        currentTitle = title;
        if (!toggles.ContainsKey(title))
            toggles.Add(title, (PluginLink.Configuration.startSettingsOpen, 0));      
        (bool, float) curToggle = toggles[title];
        currentHeight = curToggle.Item2;
        bool outcome = BeginListBox($"##{title}[config]{internalCounter++}", new Vector2(FillingWidth, currentHeight));
        if (!outcome) return false;
        if (ImGui.Button(toggles[title].Item1 ? "v" : ">", new Vector2(BarSize, BarSize)))
            curToggle.Item1 ^= true;
        canDraw = toggles[title].Item1;
        SameLine();
        NewLabel(title + $"##title{internalCounter++}", new Vector2(FillingWidthStepped() - BarSizePadded - ItemSpacingY, BarSize));
        currentHeight = BarSize + (ItemSpacingY * 2);
