...
[PersistentPetWindow]
[ModeTogglePetWindow]
public class NewPetConfigWindow : PetWindow
{
    Vector2 baseSize = new Vector2(300, 442);
    bool unsupportedMode = false;

    public NewPetConfigWindow() : base("Pet Nicknames Configuration", ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoScrollWithMouse | ImGuiWindowFlags.NoCollapse)
    {
        Size = baseSize;
    ...
    public override void OnDraw()
    {
        ...
        if (unsupportedMode)
        {
            OverrideLabel("Only INSTALLED plugins will show up!", new Vector2(FillingWidth, 24));
            BeginListBox("##SharingConfig", new Vector2(FillingWidth, 102));
            ...
            ImGui.EndListBox();
            if (Button("Back to Normal Settings", new Vector2(FillingWidth, 24)))
        ...
        else
        {
            if (Checkbox("Display Custom Names", ref PluginLink.Configuration.displayCustomNames) ||
                Checkbox("Automatically Switch Pet Modes", ref PluginLink.Configuration.automaticallySwitchPetmode))
                PluginLink.Configuration.Save();
    ...
    public override void OnDrawNormal()
    {
        if (unsupportedMode)
        {
            ...
            BeginListBox("##MinionConfig", new Vector2(FillingWidth, 194));
            OverrideLabel("Minion Specific Settings", new Vector2(FillingWidthStepped(), 25));
    ...
    public override void OnDrawBattlePet()
    {
        ...
        {
            if (!PluginLink.Configuration.understoodWarningThirdPartySettings) return;
            BeginListBox("##BattleConfig", new Vector2(FillingWidth, 194));
            OverrideLabel("Battle Pet Specific Settings", new Vector2(FillingWidthStepped(), 25));
    ...
    public override void OnDrawSharing()
    {
        ...
        {
            if (!PluginLink.Configuration.understoodWarningThirdPartySettings) return;
            BeginListBox("##SharingConfig2", new Vector2(FillingWidth, 194));
            OverrideLabel("Sharing Mode Specific Settings", new Vector2(FillingWidthStepped(), 25));
        ...
        BeginListBox("##SharingConfig3", new Vector2(FillingWidth, 194));
        OverrideLabel("Sharing Mode Specific Settings", new Vector2(FillingWidthStepped(), 25));
    ...
    public override void OnLateDraw()
    {
        ...
        if (!unsupportedMode)
        {
            if (Button("Clear All Nicknames", new Vector2(FillingWidth, 24)))
                PluginLink.Configuration.ClearAllNicknames();

            if (Button("Credits", new Vector2(FillingWidth, 24)))
                PluginLink.WindowHandler.OpenWindow<CreditsWindow>();

            if (AnyIllegalsGoingOn())
            {
                NewLine();
                if (XButton("Other Plugin Settings", new Vector2(FillingWidth, 24)))
