...
public class SerializableUserV3
{
    ...
    {
        if (mainSkeletons?.Length == 5)
        {
            this.mainSkeletons = mainSkeletons;
        }

        if (softSkeletons?.Length == 5)
        {
            this.softSkeletons = softSkeletons;
        }
    }

    [JsonConstructor]
    public SerializableUserV3(int[] ids, string[] names, string username, ushort homeworld, int[] mainSkeletons, int[] softSkeletons) 
        : this(username, homeworld, mainSkeletons, softSkeletons)
    {
