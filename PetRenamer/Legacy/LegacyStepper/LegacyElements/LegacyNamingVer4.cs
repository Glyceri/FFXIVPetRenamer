...
{
    private static readonly int[] BaseSkeletons =
    [
        -411,
        -417,
        -416,
    ...
    {
        List<SerializableUserV3> newUsers = [];

        foreach (SerializableUserV2 oldUser in configuration.serializableUsersV2!)
        {
            newUsers.Add(new SerializableUserV3(oldUser.ids, oldUser.names, oldUser.username, oldUser.homeworld, BaseSkeletons, BaseSkeletons));
        }
