...
internal unsafe class ActionMenuHook : HookableElement
{
    ...
    private void LifeCycleUpdateReplaceMenu(AddonEvent addonEvent, AddonArgs addonArgs) 
    ...
    private void UpdateActionSetting(AtkUnitBase* baseD)
    {
        ...
        if (resNode->UldManager.LoadedState != AtkLoadState.Loaded)
        {
            return;
        }
        
        if (resNode->UldManager.NodeList == null)
        {
            return;
        }
        ...
        if (resNode2->UldManager.LoadedState != AtkLoadState.Loaded)
        {
            return;
        }
        
        if (resNode2->UldManager.NodeList == null)
        {
            return;
        }
    ...
    private void UpdateReplaceMenu(AtkUnitBase* baseD)
    {
        ...
        for (int i = 0; i < list->ListLength; i++)
        {
            ...
            if (renderer->UldManager.LoadedState != AtkLoadState.Loaded)
            {
                return;
            }
            ...
            if (renderer->UldManager.NodeList == null)
            {
                continue;
            }
    ...
    private void UpdateActionMenu(AtkUnitBase* baseD)
    {
        ...
        if (baseD->UldManager.LoadedState != AtkLoadState.Loaded)
        {
            return;
        }
        
        if (baseD->UldManager.NodeList == null)
        {
            return;
        }
        ...
        for (int i = 0; i < baseD->UldManager.NodeListCount; i++)
        {
            ...
            if (node->Component->UldManager.LoadedState != AtkLoadState.Loaded)
            {
                continue;
            }
            
            if (node->Component->UldManager.NodeListCount != 11)
            {
                continue;
            }
    ...
    private bool TryAsDragAndDropNode(AtkComponentBase* atkNode, in IPettableUser user)
    {
        ...
        if (atkNode->UldManager.NodeList == null)
        {
            return false;
        }
        ...
        if (dragDropBase->UldManager.LoadedState != AtkLoadState.Loaded)
        {
            return false;
        }
        
        if (dragDropBase->UldManager.NodeList == null)
        {
            return false;
        }
