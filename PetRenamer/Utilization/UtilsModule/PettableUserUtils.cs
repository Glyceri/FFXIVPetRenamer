...
[UtilsDeclarable]
internal unsafe class PettableUserUtils : UtilsRegistryType, ISingletonBase<PettableUserUtils>
{
    public static PettableUserUtils instance { get; set; } = null!;

    public void DissectUser(PettableUser user, bool forceToInvestigate = false)
    {
        if (user == null) return;
        ...
        if (user.SerializableUser.hasCompanion || user.LocalUser || forceToInvestigate)
        {
        ...
        if (user.SerializableUser.hasBattlePet || user.LocalUser || forceToInvestigate)
        {
        ...
        if (user.MinionChanged) window.OpenForId(user.Minion.ID);
        if (user.BattlePetChanged) window.OpenForId(user.BattlePet.ID);
    }

    public BattleChara* AlterantiveFindForBChara(BattleChara* bChara, BattleChara* basePet)
    {
